...
- Prepare one of the two `slurm` scripts to claim some worker nodes on which we will launch a job.
    ...
    - **Change the line `#SBATCH --nodes={NUMBER}`** which represents the number of nodes that will process the IWP data. For the IWP workflow run in May 2023, 12+ nodes should be used. This is to avoid out-of-memory errors during staging and rasterization steps.
        - Increasing the number of nodes decreases the execution time, but we have not executed enough full IWP runs to determine the optimal number of nodes. In a past run, Robyn only rasterized 95% of the staged tiles with 10 nodes. Because of this uncertainty, a future to-do is to set up a special `rsync` script _before running the geotiff creation step_  that will continuously check for new geotiff files in the `/tmp` folder and sync them to `/scratch` _without needing to manually run an `rsync` script at intervals_. 
...
-  Adjust the file `merge_staged_vector_tiles.py` as needed:
    ...
    - Within a `tmux` session, with your virtual environment activated, and ssh's into the head node, run `python viz-workflow/merge_staged_vector_tiles.py` to consolidate all staged files to the node you specified.
