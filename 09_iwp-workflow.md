...
5. Prepare the `slurm` script to claim some worker nodes on which we will launch a job. By default, you are logged into the head node (login01, 02, or 03) which should __not__ be the node that executes the bulk of the computation (you actually cannot be charged money for this node). In the same terminal as before, run `squeue | grep {USERNAME}` (a `slurm` command) to display info about the jobs in the scheduling queue and which nodes each are running on. This list will be empty of you haven't launched any jobs yet. Then take a look at the script that will soon be run to claim the nodes: `viz-workflow/slurm/BEST-v2_gpu_ray.slurm`. (Rename this soon for clarity?) Note: lines at the top of this `slurm` script that start with `#SBATCH` are __not__ regular comments, they are settings. Other lines in the script that start with just `#` are regular comments. 
    ...
    We are not sure how much increasing the nodes decreases the time. The most time-sonsuming workflow step during her run was geotiff creation. If the job time limit is nearing, it is important to `rsync` at intervals as geotiff files are written (such as 15 min intervals). **To ask Kastan: any tips for increasing CPU usage?**
    - **To ask Robyn/Kastan: Should we set up this rsync at regular intervals in advance of the workflow? Need to ensure that files are only being transferred one direction, and of course are being removed from /tmp node as they are tranferred off. Alternatively, could set up separate 24 hour job that starts at geotiff creation stage, so for example run IWP thru staging step only, do the merge and transfer, then start new slurm job specifically to start at geotiff to maximize time for that step to allow for low CPU usage.** 
    ...
    - Look at the line `#SBATCH --account={ACCOUNT NAME}` and enter the account name for the appropriate allocation. Note that our allocations come with GPU and CPU counterparts that are billed separately. We do __not__ store these private account names on GitHub, so pay attention to this line when you are pushing. **To ask Robyn/Kastan: Kastan used GPU in our meeting, when would we use CPU?**
...
10. Adjust `viz-workflow/PRODUCTION_IWP_CONFIG.py` as needed:
    ...
    - The output path will __always__ need to be changed, as we retain the outputs of past runs and do not overwrite them. Change the path following `/scratch/{ALLOCATION}/julietcohen`.
...
**To do: run rsync command to `rsync` files from the one consolidated /tmp to /scratch as they are processed (like at regular time intervals) so none are lost at 24 hrs**
