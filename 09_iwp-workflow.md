...
- Transfer all geotiff files from `/tmp` to `/scratch` by first opening `rsync_merge_raster_to_scratch.py` and updating the variable `output_dir` to the correct string, Then run `python rsync_merge_raster_to_scratch.py`. This script both merges the rasterization outputs in the sense that it moves them all to the same directory, followng the z-evel tile hierarchy format. The individual files do not need to be merged, but they need to exist within the same tile hierarchy on disk, within `/scratch`.
